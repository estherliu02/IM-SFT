...
def parse_args():
    ...
    parser.add_argument(
        "--lora_target",
        type=str,
        default="q_proj,o_proj,v_proj,k_proj,gate_proj,up_proj,down_proj",
        help="Comma-separated list of target modules for LoRA adaptation.",
    )
    parser.add_argument(
        "--use_flash_attn",
        action="store_true",
    ...
    parser.add_argument(
        "--enable_liger_kernel",
        action="store_true",
        help="Enable Liger kernel for efficient attention with very long sequences."
    )
    parser.add_argument(
        "--tokenizer_name",
        type=str,
...
def main():
    ...
    if args.tokenizer_name:
        tokenizer = AutoTokenizer.from_pretrained(
            args.tokenizer_name, trust_remote_code=args.trust_remote_code, use_fast=not args.use_slow_tokenizer)
    elif args.model_name_or_path:
        tokenizer = AutoTokenizer.from_pretrained(
            args.model_name_or_path, trust_remote_code=args.trust_remote_code, use_fast=not args.use_slow_tokenizer)
    else:
        raise ValueError(
    ...
    if args.use_lora:
        ...
        # Parse the target modules from comma-separated string
        target_modules = [module.strip() for module in args.lora_target.split(",")]
